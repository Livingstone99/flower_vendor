...
export default function FlowersPage() {
  ...
  const loadProducts = async () => {
    setLoading(true);
    const data = await getProducts({ kind: "bouquet" });
    setProducts(data);
    setLoading(false);
  };
  ...
  return (
    <main className="max-w-[1440px] mx-auto px-4 md:px-10 lg:px-20 py-8">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <span className="text-[#111812] dark:text-white text-sm font-semibold">Flowers</span>
      </div>
      ...
      <div className="flex flex-wrap justify-between items-end gap-4 mb-8">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[#618965] dark:text-primary/70 text-base font-normal">
            Beautiful bouquets and arrangements for every occasion. Fresh, hand-picked, and delivered with care.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <p className="text-gray-600 mb-4">No flowers found.</p>
          <Link
            href="/shop"
